...
ggplot(summary_df, aes(x = Attitude_Label, y = mean_pct, fill = Condition)) +
  ...
  geom_errorbar(aes(ymin = mean_pct - se, ymax = mean_pct + se),
                ...
                width = 0.1,
                color = "grey40") +
  scale_fill_manual(values = c("Label Shown" = "orange", "Control (Without Label)" = "darkblue")) +
...
When participants are given a choice, they are less likely to watch a video that argues against their beliefs if it is labeled as AI-generated versus not knowing how it is created. This findings suggest that labeling a deepfake as such reduces
people's willingness to voluntarily exposure, making its content less appealing. However, as shown in Study 1, AI label does not prevent people from being influenced by the content when they are exposed to it. AI label may help reducing the influence of content created by AI in reducing its exposure, but not its influence once people choose to watch it[^1]. 
